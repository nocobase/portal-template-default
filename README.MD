...
`pnpm build:client` emits browser assets to `dist/client`. `pnpm build:server`
emits the ESM server build, production package manifest, and server runtime
dependencies to the `dist` package root.
